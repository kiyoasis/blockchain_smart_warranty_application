...
contract SmartWarranty {
    ...
    // function testRegisterBatteryInfo() public {
    //     registerBatteryInfo("testBattery1", 0x14723a09acff6d2a60dcdf7aa4aff308fddc160c, 0xca35b7d915458ef540ade6068dfe2f44e8fa733c, 570, 100, 2019, 3);
    //     registerBatteryInfo("testBattery2", 0x4b0897b0513fdc7c541b6d9d7e929c4e5364d2db, 0xca35b7d915458ef540ade6068dfe2f44e8fa733c, 80, 100, 2019, 4);
    // }
    
    // function testAADE() public {
    //     inputAnnualAccumulatedDischargeEnergy(1, 100);
    // }

    // The annual average SOC condition
    // function inputAnnualAverageSOCInfo(uint _tokenId, uint year, uint acceptanceDate, uint endDate, uint socValue) public {
    ...
    //     //AnnualAverageSOC memory socObj = AnnualAverageSOC(year, acceptanceDate, endDate, socValue, violationFlag);
    //     AnnualAverageSOC memory socObj = AnnualAverageSOC(year, socValue, violationFlag);
    //     tokenIdToAnnualAverageSOCList[_tokenId].push(socObj);
        
    //     tokenIdToBatteryInfo[_tokenId].annualAverageSOC = socObj;
    //     //tokenIdToBatteryInfo[_tokenId].yearToAnnualAverageSOCInfo[year] = socObj;
    //     tokenIdToBatteryInfo[_tokenId].warrantyAnalysisInfo.annualAverageSOCViolation = violationFlag;
    // }
    
    // function inputAnnualAverageSOCInfo(uint _tokenId, uint year, uint socValue) public {
    //     require(tokenId > _tokenId);
    //     AnnualAverageSOC memory socObj = AnnualAverageSOC(year, socValue, false);
    //     //tokenIdToAnnualAverageSOCList[_tokenId].push(socObj);
    //     tokenIdToBatteryInfo[_tokenId].annualAverageSOC = socObj;
    //     //tokenIdToBatteryInfo[_tokenId].yearToAnnualAverageSOCInfo[year] = socObj;
    // }
    ...
    //TODO
    // The average hourly temperature condition
    /* function inputAverageHourlyTemperatureInsideBatteryEnclosure(uint _tokenId, uint averageHourlyTemperatureInsideBatteryEnclosure, uint moduleTemperature, uint averageDailyBatteryEnclosureTemperature) public {
        
        require(tokenId > _tokenId);
        
        bool violationFlag = false;
        
        // If the average daily battery enclosure temperature is less than max_average_enclosure_temp
        if (averageDailyBatteryEnclosureTemperature < max_average_enclosure_temp) {

            // If one or more module temperatures are greater than min_module_temp
            if (moduleTemperature > min_module_temp) {
                if (averageHourlyTemperatureInsideBatteryEnclosure < abs_min_enclosure_temp || max_enclosure_temp < averageHourlyTemperatureInsideBatteryEnclosure) {
                    violationFlag = true;
                }
            } 
            
            // If all of the module temperatures are less than min_module_temp
            else {
                if (averageHourlyTemperatureInsideBatteryEnclosure < nom_min_enclosure_temp || max_enclosure_temp < averageHourlyTemperatureInsideBatteryEnclosure) {
                    violationFlag = true;
                }
            }
        ...
        // If the average daily battery enclosure temperature is more than max_average_enclosure_temp
        else {
            violationFlag = true;
        }
        
        BatteryInfo storage bInfo = tokenIdToBatteryInfo[_tokenId];
        bInfo.warrantyAnalysisInfo.averageHourlyTemperatureViolation = violationFlag;
    } */
    ...
    // function showVariables() constant returns (uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint, uint) {
    //     return (
    //         max_average_soc,
    //         abs_min_enclosure_temp,
    //         nom_min_enclosure_temp,
    //         max_enclosure_temp,
    //         min_module_temp,
    //         max_average_enclosure_temp,
    //         peak_dc_rack_power,
    //         daily_avg_rack_power,
    //         abs_max_storage_temp,
    //         max_module_oper_temp,
    //         annual_accum_disch_energy,
    //         soc_penalty_factor,
    //         avg_temp_penalty_factor_1,
    //         avg_temp_penalty_factor_2,
    //         avg_temp_penalty_factor_3,
    //         peak_power_penalty_factor,
    //         avg_power_penalty_factor,
    //         max_dod
    //     );
    // }
